...
export default class TagsPlugin {
  ...
  getTags() {
    ...
    Array.from(tagElements).forEach(tagElement => {
      ...
      console.log(tagElement, dataset)

      if (!foundTags[dataset.tagtype]) foundTags[dataset.tagtype] = [];
      foundTags[dataset.tagtype].push(this.tagData[dataset.tagtype].tags.find(tag => tag.key === dataset.tagkey));
    });
...
function mockData() {
  return {
    ...
    $: [
      ...
      { label: 'ETH', value: 1, image: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Ethereum-icon-purple.svg/1200px-Ethereum-icon-purple.svg.png' },
      { label: 'SOL', value: 2, image: 'https://vectorlogo4u.com/wp-content/uploads/2021/09/solana-logo-vector-01.png' },
      { label: 'ADA', value: 3, image: 'https://ak.picdn.net/shutterstock/videos/1007246662/thumb/6.jpg' },
      { label: 'DAG', value: 4, image: 'https://constellationnetwork.io/wp-content/uploads/2021/04/Logo.png' },
    ],
  };
}
