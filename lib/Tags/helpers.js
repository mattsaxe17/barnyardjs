...
import { PositionContentSearcher, addRangeToSelection } from 'roosterjs-editor-dom';
...
// --------------------------------------------------------------------------------- //
function addTag(searchRange, tag) {
  const tagType = this.content[searchRange.searchStart];
  const tagNode = document.createElement('span');
  tagNode.classList.add('barnyardjs-editor-tag');
  tagNode.innerText = tagType + tag.label;
  tagNode.innerHTML += '&nbsp;&nbsp;';
  tagNode.setAttribute('contenteditable', false);
  tagNode.setAttribute('data-tagtype', tagType);
  tagNode.setAttribute('data-tagkey', tag.key);
  ...
  document.querySelector(`[${this.uniqueHash}] .barnyardjs-editor`).innerHTML =
    editor.innerHTML.substring(0, searchRange.searchStart) +
    tagNode.outerHTML +
    '&nbsp;' +
    editor.innerHTML.substring(searchRange.searchEnd + 1);
...
// -------------------------------------------------------------- //
// ----- Gets the key of a dropdown row from a click event ----- //
// ------------------------------------------------------------ //
function getKey(e) {
  if (e.target.classList.contains('barnyardjs-editor-tag-dropdown-row')) {
    return e.target.dataset.key;
  } else {
    return e.target.parentElement.dataset.key;
  }
}
...
function attachDropdown(tags, searchRange) {
  ...
  tags?.forEach(tag => {
    ...
    dropdownRow.onclick = e => addTag.call(this, searchRange, tag);
  });
...
function filterDropdownOptions(tags, searchRange, dropdown) {
  ...
  currentDropdownOptions.forEach((row, ind) => {
    ...
    row.onclick = e => addTag.call(this, searchRange, tags.find(tag => tag.key === getKey(e)));
  });
}
...
// ----------------------------------------------- //
// ----- Selects first item in the dropdown ----- //
// --------------------------------------------- //
export function selectFirst(tags, searchRange) {
  addTag.call(this, tags[0].label, searchRange);
  removeDropdowns();
}

// ---------------------------------------------------------------- //
// ----- Checks to see if the seach string is an exact match ----- //
// -------------------------------------------------------------- //
export function checkForExactMatch(tags, searchStr) {
  if (processString(tags[0].label) === processString(searchStr)) return true;
  return false;
}
