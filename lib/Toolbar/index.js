...
export default class ToolbarPlugin {
  ...
  setSelectActions() {
    document
      .querySelectorAll(`[${this.uniqueHash}] select`)
      .forEach(select => {
        select.onchange = e => {
  ...
  setButtonActions() {
    document
      .querySelectorAll(`[${this.uniqueHash}] button`)
      .forEach(button => {
        button.onclick = e => {
  ...
  setColorSelectorActions() {
    document
      .querySelectorAll(`[${this.uniqueHash}] input[type="color"]`)
      .forEach(input => {
        input.onchange = e => {
  ...
  wrapEditor() {
    ...
    el.removeAttribute(this.uniqueHash);
